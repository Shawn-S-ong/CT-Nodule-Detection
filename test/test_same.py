...
if __name__=='__main__':
    ...
    sliceim, origin, spacing, isflip = load_itk_image(
        "C:\\Users\\s4548361\\Desktop\\dataset\\" + title + ".mhd")
    ...
    margin = 5
    extendbox = np.vstack(
        [np.max([[0, 0, 0], box[:, 0] - margin], axis=0), np.min([newshape, box[:, 1] + 2 * margin], axis=0).T]).T
    ...
    sliceim1 = sliceim1[np.newaxis, ...]

    sliceim = sliceim1

    np.save(os.path.join("C:/Users/s4548361/Desktop/dataset/Preprosessing",title +'_lbb.npy'), sliceim)
